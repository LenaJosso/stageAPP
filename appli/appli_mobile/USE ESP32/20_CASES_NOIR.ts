    ...
    try {
      ...
      subscription.current = currentDevice.monitorCharacteristicForService(
        ...
        (e, characteristic) => {
          ...
          // Sécurité : On s'assure de lire le premier octet proprement
          // et on applique un modulo 20 pour éviter tout débordement au-delà du quartier 20
          const rawCounter = bytesNotify.readUInt8(0);
          const counter = rawCounter % 20;

          setState((s) => {
            // Si l'application a déjà choisi une LED cible pour ce quartier (ex: Gros Lot, Bankrupt)
            if (
              s.targetLedIndexGlobal !== null &&
              Math.floor(s.targetLedIndexGlobal / 3) === counter
            ) {
              return {
                ...s,
                isSpinning: true,
